...
async def list_films(
    ...
    sort_by: str | None = Query("episode_id", description="Field to sort by (title, episode_id, release_date)"),
    ...
    try:
        ...
        films = [Film.from_swapi(data, data.get("id", i + 1)) for i, data in enumerate(all_films_data)]
...
async def get_film_planets(film_id: int) -> list[PlanetSummary]:
    ...
    try:
        ...
        return [
            PlanetSummary.from_swapi(data, data.get("id", 0))
